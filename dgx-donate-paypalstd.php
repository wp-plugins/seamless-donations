...
function dgx_donate_paypalstd_ajax_checkout () {
	...
	dgx_donate_debug_log ( 'Payment process checkout started.' );
	...
	if( get_transient ( $sessionID ) !== false ) {
		dgx_donate_debug_log ( 'Session ID: ' . $sessionID );
		...
		dgx_donate_debug_log ( 'Payment process data assembly.' );
		...
		dgx_donate_debug_log ( 'Session ID: ' . $postData['SESSIONID'] );
		dgx_donate_debug_log ( 'Nonce: ' . $_POST['nonce'] );
