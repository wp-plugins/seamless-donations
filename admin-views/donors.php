...
class Dgx_Donate_Admin_Donors_View {
	...
	function menu_page() {
		...
		echo "<h2>" . esc_html__( 'Donors', 'dgx-donate' ) . "</h2>\n";
		...
		echo "<h3>" . esc_html( sprintf( __( 'Donor Report for %1$s to %2$s', 'dgx-donate' ), $start_date, $end_date ) ) . "</h3>\n";
		...
		if ( count( $my_donations ) ) {
			...
			echo "<tr>";
			echo "<th>" . esc_html__( 'Donor/Date', 'dgx-donate' ) . "</th>";
			...
			foreach ( (array) $my_donor_emails as $my_donor_email => $donor_donation_ids ) {
				...
				echo "<td>" . esc_html( $formatted_donor_total ) . "</td>";
				echo "</tr>\n";
			...
			echo "<td>" . esc_html( $formatted_grand_total ) . "</td></tr>\n";
			
			echo "</tbody></table>\n";
		} else {
			echo "<p>" . esc_html__( 'No donors found.', 'dgx-donate' ) . "</p>\n";
		}
		...
		echo "<h3>" . esc_html( 'Date Range', 'dgx-donate' ) . "</h3>\n";
		...
		echo "<p>" . esc_html__( 'Start Date', 'dgx-donate' ) . ": ";
		...
		echo "</p>";
		echo "<p>";
		echo "<input id='submit' class='button' type='submit' value='" . esc_attr__( 'Update', 'dgx-donate' ) . "' name='submit'></p>\n";
		echo "</form>";
