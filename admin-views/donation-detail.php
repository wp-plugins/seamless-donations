...
class Dgx_Donate_Admin_Donation_Detail_View {
	static function show( $donation_id ) {
		...
		echo "<h2>" . esc_html__( 'Donation Detail', 'dgx-donate' ) . "</h2>\n";
		...
		if ( ! $donation_deleted ) {
			...
			echo "<h3>" . esc_html__( 'Donation Details', 'dgx-donate' ) . "</h3>\n";
			...
			if ( ! empty( $add_to_mailing_list ) ) {
				$add_to_mailing_list = __( 'Yes', 'dgx-donate' );
			} else {
				$add_to_mailing_list = __( 'No', 'dgx-donate' );
			}
			echo "<tr><th>" . esc_html__( 'Add to Mailing List?', 'dgx-donate' ) . "</th>";
			echo "<td>" . esc_html( $add_to_mailing_list ) . "</td></tr>\n";

			$anonymous = get_post_meta( $donation_id, '_dgx_donate_anonymous', true );
			if ( empty( $anonymous ) ) {
				$anonymous = __( 'No', 'dgx-donate' );
			} else {
				$anonymous = __( 'Yes', 'dgx-donate' );
			}
			echo "<tr><th>" . esc_html__( 'Would like to remain anonymous?', 'dgx-donate' ) . "</th>";
			...
			if ( ! empty( $tribute_gift ) ) {
				$tribute_gift_message = __( 'Yes', 'dgx-donate' ) . " - ";
				...
				if ( empty( $memorial_gift ) ) {
					...
					$tribute_gift_message .= __( 'in memory of', 'dgx-donate' ). ' ';
				}
				
				$tribute_gift_message .= $honoree_name . "<br/><br/>";
				if ( 'TRUE' == $honor_by_email ) {
					$tribute_gift_message .= __( 'Send acknowledgement via email to', 'dgx-donate' ) . '<br/>';
			...
			if ( $is_repeating_donation ) {
				...
				echo "<h3>" . esc_html__( 'Related Donations', 'dgx-donate' ) . "</h3>\n";
				echo "<p class='description'>";
				echo esc_html__( 'For repeating donations, displays a list of other donations in the series (subscription)', 'dgx-donate' );
				echo "</p>\n";
				// Show the array
				echo "<table class='widefat'><tbody>\n";
				if ( count( $related_donation_ids ) ) {
					echo "<tr>";
					echo "<th>" . esc_html__( 'Date', 'dgx-donate' ). "</th>";
					echo "<th>" . esc_html__( 'Transaction ID', 'dgx-donate' ) . "</th></tr>";
					...
					echo "<tr>";
					echo "<th>" . esc_html__( 'No related donations found', 'dgx-donate' ) . "</th>";
					echo "</tr>\n";
				}
				echo "</tbody></table>\n";
			}
			...
			echo "<h3>" . esc_html__( 'Delete this Donation', 'dgx-donate' ) . "</h3>";
			echo "<p>" . esc_html__( 'Click the following button to delete this donation.  This will also remove this donation from all reports.  This operation cannot be undone.', 'dgx-donate' ) . "</p>";

			if ( $is_repeating_donation ) {
				echo "<p><strong>" . esc_html__( 'This is a repeating donation (subscription).  Deleting this donation does NOT end the subscription.  The donor will need to log into PayPal to end the subscription.', 'dgx-donate') . "</strong></p>";
			}
			...
			echo "<p><input class='button' type='submit' value='" . esc_attr__( 'Delete Donation', 'dgx-donate' ) ."'";
			echo " onclick=\"return confirm('" . esc_attr( 'Are you sure you want to delete this donation?', 'dgx-donate' ) . "');\"></p>\n";
			echo "</form>";
