...
class Dgx_Donate_Admin_Donations_View {
	...
	function menu_page() {
		...
		echo "<h2>" . esc_html__( 'Donations', 'dgx-donate' ) . "</h2>\n";
		...
		echo "<h3>" . esc_html( sprintf( __( 'Donation Report for %1$s to %2$s', 'dgx-donate' ), $start_date, $end_date ) ) . "</h3>\n";
		...
		if ( count( $my_donations ) ) {
			...
			foreach ( (array) $my_donations as $my_donation ) { 
				...
				if ( $ok_to_add ) {
					$designated_fund = __( 'Undesignated', 'dgx-donate' );
			...
			echo "<tr>";
			echo "<th>" . esc_html__( 'Fund/Date', 'dgx-donate' ) . "</th>";
		...
		if ( count( $my_donations ) ) {
			...
			echo "<h3>" . esc_html__( 'Export Report as Spreadsheet (CSV)', 'dgx-donate' ) ."</h3>\n";
			echo "<p>" . esc_html__( 'Click the following button to export detailed information for each donation in this report to a comma-separated-value (CSV) file compatible with most spreadsheet software.', 'dgx-donate' ) . "</p>";
			...
			echo "</p><p>";
			echo "<input id='submit' class='button' type='submit' value='" . esc_attr__( 'Export Report', 'dgx-donate' ) . "' name='submit'></p>\n";
			echo "</form>";
			echo "<hr/>";
		}

		echo "<h3>" . esc_html__( 'Date Range', 'dgx-donate' ) . "</h3>\n";
		...
		echo "<p>";
		echo esc_html__( 'Start Date', 'dgx-donate' ) . ": ";
		echo "<input type='text' name='startdate' value='" . esc_attr( $start_date ) . "' size='12'/>";
		echo "<br/>";
		echo esc_html__( 'End Date', 'dgx-donate' ) . ": ";
		echo "<input type='text' name='enddate' value='" . esc_attr( $end_date ) . "' size='12'/>";
		echo "</p>";
		echo "<p>";
		echo "<input id='submit' class='button' type='submit' value='" . esc_attr__( 'Update', 'dgx-donate' ) . "' name='submit'>";
		echo "</p>\n";
		echo "</form>";
